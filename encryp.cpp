...
int main() {
    ...
    try {
        RC4 cipher;
        std::cout << "Reading file.txt..." << std::endl;
        std::vector<unsigned char> buffer = readFile(targetFile);
        // hemmm i am feeling somthing new thing , insa camp was best , i like it and i missed it offffff

       // std::cout << "Encrypting file contents..." << std::endl;
        cipher.process(key, buffer);

        //std::cout << "Overwriting file.txt with encrypted data..." << std::endl;
